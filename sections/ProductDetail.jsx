...
export default function ProductDetail() {
  ...
  const { variant, onSelectOption, isOptionSelected } =
    useSelectedVariant(product);
  const { add, remove, set } = useCartItemQuantity(product, variant);

  const [quantity, setQuantity] = useState(1);

  // console.log("onSelectOption object", onSelectOption);
                          ...
                          {option.values.map((value, j) => (
                            <li
                              ...
                              className={classNames(
                                "border border-gray-300 rounded-md px-4 py-2 text-sm cursor-pointer transition duration-200 ease-in-out",
                                {
